...
curl "https://homeservicedata.com/api/v1/hardware/inverters?manufacturer=Enphase&per_page=50" \
  -H "x-api-key: YOUR_API_KEY"
```

```bash Partial SKU search
curl "https://homeservicedata.com/api/v1/hardware/inverters?code=IQ8&is_default=true" \
  -H "x-api-key: YOUR_API_KEY"
```

```bash Page 2 with custom page size
curl "https://homeservicedata.com/api/v1/hardware/inverters?page=2&per_page=10" \
  -H "x-api-key: YOUR_API_KEY"
```
</CodeGroup>
